...
export default function RegisterPage() {
  ...
  const handleGoogle = async () => {
    try { await signInWithGoogle(); toast.success('Welcome!'); router.push('/'); }
    catch (err: any) { toast.error(err.message || 'Google sign-in failed'); }
  };

  const handleApple = async () => {
    try { await signInWithApple(); toast.success('Welcome!'); router.push('/'); }
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-dark-700/50 border border-white/10 rounded-2xl p-8">
        ...
        <p className="text-gray-400 text-center mb-8">Join Mo-Tech today</p>
