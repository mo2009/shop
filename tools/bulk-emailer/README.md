...
- **Individual**: the program creates a separate Outlook `MailItem`
  ...
  see each other.
- **CC**: a single `MailItem` is created with the first recipient in
  `To:` and the rest in `Cc:`. Everyone sees the full distribution.
...
| `storage.py` | JSON persistence for presets and settings |
